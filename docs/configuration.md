...
```text
~/.config/beef-xhs-library/config.toml
```

上一版的 `~/.config/xhs-library` 和更早的 `~/.config/xhs-tikhub-feishu-ingest` 配置目录继续兼容；首次使用新名称时会自动迁移到 `~/.config/beef-xhs-library`。

## 环境变量

| 环境变量 | 用途 |
| --- | --- |
| `BEEF_XHS_LIBRARY_CONFIG` | 指定配置文件路径 |
| `BEEF_XHS_LIBRARY_ROOT` | 指定本地内容库目录 |
| `XHS_LIBRARY_CONFIG` | 上一版配置路径变量，继续兼容 |
| `XHS_LIBRARY_ROOT` | 上一版本地目录变量，继续兼容 |
...
```bash
./beef-xhs-library doctor --config /path/to/config.toml
./beef-xhs-library run "<小红书链接>" --config /path/to/config.toml
```
